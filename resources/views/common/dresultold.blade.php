...
function change(){location.href=LIBS.url({'lottery':$('#lottery').val(),'date':$('#date').val(),'table':$('#tb').val()});}
$(function(){$('#date').change(change).datepicker();$('#tb').val('');})
    </script>
</head>
...
<div class="main">
    <div class="top_info">
        ...
        <span class="right">
彩种：<select id="lottery" name="lottery" onchange="change()">
@foreach($lotterys as $lot)
                    <option value="{{$lot['lottery']}}"
                            @if($lot['lottery'] == $lottery) selected @endif>{{$lot['gname']}}</option>
                @endforeach
</select>

日期：<input id="date" value="{{$date}}" class="hasDatepicker">
</span>
    </div>
