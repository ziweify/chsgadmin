    ...
    <div class="contents">
        <table class="data_table list">
            ...
            <tr>
                ...
                <th class="range th-box" style="width:210px">
                    <div class="display_amount">
                        下注金额
                    </div>
                    <div class="input_amount" style="display:none">
                        <input type"search" placeholder="最小下注金额" name="minAmount1" value="{{$minAmount}}" style="width:80px" />&nbsp;
                        <input type"search" placeholder="最大下注金额" name="maxAmount1" value="{{$maxAmount}}" style="width:80px"/>
                    </div>
                ...
                @if($tax==1)<th class="result">{{$tax_name}}</th>@endif
                <th>占成明细</th>
            </tr>
            </thead>
            <tbody>
            @foreach($bao as $vo)
                <tr>
                    <td>
                        <a href='javascript:queryBetExtraDetail("{{$vo['user']}}", "{{$vo['code']}}", "{{$vo['lottery']}}","{{$vo['dates']}} 00:00:00")'>{{$vo['code']}}#</a>
                    ...
                    <td class="money dividend color">@if($vo['z'] == 9)<span>未结算</span>@else{{$vo['rs']}}@endif</td>
                    <td class="share">{{$vo['mezc']}}%</td>
                    <td class="money color">@if($vo['z'] == 9)@else{{$vo['mers']}}@endif</td>
                    @if($tax==1)<td>{{$vo['tax']}}</td>@endif
                    <td class="detail"><a bid="{{$vo['code']}}" drawDate="{{$vo['dates']}} 00:00:00">明细</a></td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
    ...
    <div class="page">
        <div class="page_info">
            ...
            <span class="page_control">
                <a class="first" href="?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page=1">首页</a> |
                ...
                <a class="previous" @if($page-1 >= 1)href="?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page={{$page-1}}"@endif>前一页</a>『
                 @for($i=$startpcount;$i<=$endpcount;$i++)
                    @if($i == $page)
                        <span class="current">{{$i}}</span>&nbsp;
                    @else
                        @if($i == $pcount)
                            <a href='?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page={{$i}}' class="page">{{$i}}</a>
                        @else
                            <a href='?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page={{$i}}' class="page">{{$i}}</a>&nbsp;
                        @endif
                    @endif
                @endfor
                』<a class='next' @if($page+1 <= $pcount)href='?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page={{$page+1}}'@endif>后一页</a>&nbsp;|
                <a class='next' href='?game={{$game}}&minAmount={{$minAmount}}&maxAmount={{$maxAmount}}&lottery={{$lottery}}&date={{$date}}&username={{$username}}&begin={{$begin}}&end={{$end}}&settle={{$settle}}&page={{$pcount}}'>末页</a>
            </span>
            @endif
        </div>
    </div>
