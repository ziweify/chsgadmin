...
<table class="data_table info_table user_panel edittb list">
	...
	<tbody>
		...
		<tr class="hides">
			<th>额度模式：</th>
			<td> <label><input type="radio" name="fudong" value="0" class="fudong" @if($fudong==0)checked="checked"
						@endif disabled />信用模式</label>
				<label><input type="radio" name="fudong" class="fudong" value="1" @if($fudong==1)checked="checked" @endif
						disabled />现金模式</label>
			</td>
		</tr>
		<TR class="modetr" style="display: none;">
			...
			<TD><input type="text" name="maxmoney" id='maxmoney' class="input hide"
					...
					class="hide">
					&nbsp;&nbsp;[限额
					<label>{{$fidmaxmoney}}</label>
					]&nbsp;&nbsp;可用余额：
					<label id='money'>{{$money}}</label></span>&nbsp;<a href='javascript:void(0);'>修改</a></td>
		</tr>
		...
		@if($layer<$maxlayer) @if($maxrenflag==1) <TR>
			...
			<tr>
				...
				<td>
					<div>
						@if($moneypassflag==1)已绑定@else未绑定@endif @if($moneypassflag==1)<input type="button"
							class="czmoneypass" value="重置">@endif
					</div>
				</td>
			</tr>
			<TR>
				...
				<TD>
                    @foreach($fidpan as $vo)
						<input class="pantype" type="checkbox"  @if(in_array($vo,$pans))checked @endif value="{{$vo}}" @if($dis==0)disabled @endif />{{$vo}}盘
					@endforeach
					<select name="defaultpan" id='defaultpan' class="hide">
                        @foreach($pans as $vo)
						<option value="{{$vo}}" @if($defaultpan==$vo)selected @endif>{{$vo}}盘</option>
						@endforeach
					</select>
				</td>
			</tr>
...
<table class="data_table info_table share_panel input_panel addtb2 list">
	...
	<tbody>
		...
		<tr @if($vo['ifok']==0)style="display:none" @endif>
			...
			<td>
				<input type="text" class="upzc share" maxzc='{{$vo['zc']}}' value='{{$vo['uupzc']}}' />(0% 至
				{{$vo['zc']}}%)
			</td>
			<td>
