<table class="data_table info_table user_panel addtb list">
	...
	<tbody>
		...
		<TR>
			<th>登入密码：</th>
			<TD style='display: flex;align-items: center;'><input style="margin-right: 5px;" type="text" name=password id=password class="input" /> <input type="hidden" name=userpass
		...
		<TR>
			...
			<TD>
                @foreach($pan as $vo)
				<input class="pantype" type="checkbox" value="{{$vo}}" />{{$vo}}盘
				@endforeach
