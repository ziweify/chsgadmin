...
<table class="data_table info_table user_panel edittb list">
	...
	<tbody>
		...
		<TR>
			<th>会员名称：</th>
			<TD style='display: flex;align-items: center;'><input style="margin-right: 5px;" type="text" name='name' id='name' class="input" value='{{$name}}' /></td>
		</tr>
		<tr>
			<th>额度模式：</th>
		...
		@if($layer<$maxlayer) @if($maxrenflag==1) <TR>
			...
			<tr>
				...
				<td>
					<div>
						@if($moneypassflag==1)已绑定@else未绑定@endif @if($moneypassflag==1)<input type="button" class="czmoneypass" value="重置">@endif
					</div>
				</td>
			</tr>
			<TR>
				...
				<TD>
					...
					<input class="pantype"  @if(in_array($vo,$pans)) checked @endif type="checkbox" value="{{$vo}}" @if($dis==0)disabled @endif />
					{{$vo}}盘
					@endforeach
