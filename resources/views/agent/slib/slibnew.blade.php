...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link href="/xypone/css/default/betslib.css" rel="stylesheet" type="text/css" />
		<!--<link href="/css/default/uitz.css" rel="stylesheet" type="text/css" />-->
		<script language="javascript" src="/xypone/default/js/default/jsagent/slibnewagent.js?v=11121"></script>
		<style type="text/css">
			.flys {
				background: yellow
			}
	...
	<body class="skin_blue">
		...
		<div class="main">
			<ul class="menu_sub">
				<li class="menu_sub_title">当前选中：<span id="lotteryType">{{$gname}}</span></li>
				<li id="XYFT" class="menu_sub_links">
                    @foreach($b as $key=>$vo)
					<a bid="{{$vo['bid']}}" href="javascript:void(0)" target="frame" class="@if($key==0)elected @endif n{{$vo['bid']}}">{{$vo['name']}}</a> @if($key != count($b)-1)|@endif
					@endforeach
				</li>
			</ul>
			<div class="top_info">
				<span id="drawNumber" class="title">&nbsp;<label>{{$qishu[0]}}</label>期</span>
				<input type="hidden" id="qishu" value="{{$qishu[0]}}"/>
				<div class="op">
					...
					<select id='userid' layer='{{$layer}}' style="display:none">
                    ...
                    @if($layer==1 && $ifexe==1)
					<!-- <input type="button" class="btn3 btnf psetbtn" value="赔率設置" /> -->
					@endif
					<input type="button" class="btn1 downfast btnf" style="display: none;" value="下载未结算" />
				</div>
				<div class="right" style="margin-right: 2px">
					...
					<label class="time" style='color: black'></label>秒
            ...
            @if($layer==1 && $ifexe==1)
			<div class="top_info psets">
                @if($layer==1 && $ifexe==1)
				<span>显示:</span>
				<select class="plmode">
					<option value="1">系统赔率</option>
					<option value="3" selected>当前赔率</option>
					@if($pself!=1)<option value="2">调节值</option>
                    @else<option value="4">默认赔率</option>
					@endif
				</select>

                @if($pself==1)
				<input type="button" class="btn3 btnf moren" value="写入默认" ac='writemoren' />
				<input type="button" class="btn3 btnf moren" value="恢复默认" ac='resetmoren' />@endif
				<input type="button" class="btn3 btnf moren @if($pself==1)hide @endif" value="清零"
...
<input type="button" class="calc button" value="计算补货"> -->
                    ...
                    @if($flytype==1)
                    <select id='fly'><option value="1">内补</option></select>
                    @endif
                    @if($flytype==2)
                    <select id='fly'><option value="2">外补</option></select>
                    @endif
                    @if($flytype==3)
                    <select id='fly'>
                    ...
                    >@endif
					@if($flytype==1 || $flytype==3) <input type="button" class='button' id='pfly' value="批量补货" />@endif
				</span>
			</div>
		</div>
		<table class="xxs data_table list xxtb">
			<thead>
				<tr>
					...
					<th>
						<a href="javascript:void(0);" class="time ts1">投注时间<img style="margin-left: 3px;" src="/xypone/default/img/down.gif"
								s='down' /></a>
					</th>
					<th>
						种类
					...
					<th>
						<a href="javascript:void(0);" class="je ts1">金額<img style="margin-left: 3px;" src="/xypone/default/img/down.gif" s='up' /></a>
					</th>
					<th>
						退水(%)
					</th>
		...
		<table class="xxs data_table list flytb">
			<thead>
				<tr>
					...
					<th>
						<a href="javascript:void(0);" class="time ts1">投注时间<img src="/xypone/default/img/down.gif"
								s='down' /></a>
					</th>
					<th>
						投注种类
					...
					<th>
						<a href="javascript:void(0);" class="je ts1">金額<img src="/xypone/default/img/down.gif" s='up' /></a>
					</th>
					<th>
						退水(%)
					</th>
