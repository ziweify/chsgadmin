...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript" src="/xypone/default/js/default/jsagent/autoflyagent.js?v=11111"></script>
		<style type="text/css">
			.data_table .flag {
				width: 70px;
			}
	...
	<body id="topbody">
		<div class="main">
			<div class="top_info">
				<span class="title">{!! $username !!}</span><span class="right"></span>
			</div>
			<form style="margin:0;padding:0" method="post">
							...
							<tr>
								<td class="panel">
									<table class="list data_table at_0">
										...
										<tbody>
                                            @foreach($fly as $i=>$vo)
											@if($i<=$nums) <TR class='nr' f='{{$vo['ftype']}}'
												ab='{{$vo['ab']}}'>
												<td style="display: none;"><input type="checkbox"
														class="pantbc" /><input type="button" class="s1 pantb"
														value="同步选中" /></td>
												<th>{{$vo['name']}}</th>
												<td>
                                                    @if($vo['ab']==1)
												...
												<td>
													<input type="radio" class='ifok' name="ifok{{$vo['ftype']}}"
														value="1" @if($vo['ifok']==1) checked @endif />启用
													<input type="radio" class='ifok' name="ifok{{$vo['ftype']}}"
														value="0" @if($vo['ifok']==0) checked @endif />停用
												</td>
							</TR>
							@endif
							@endforeach
						...
						<table class="list data_table at_0">
							...
							<tbody>
								...
								<TR class='nr' f='{{$vo['ftype']}}' ab='{{$vo['ab']}}'>
									<td style="display: none;"><input type="checkbox" class="pantbc" /><input
											type="button" class="s1 pantb" value="同步选中" /></td>
									<th>{{$vo['name']}}</th>
									<td>
                                        @if($vo['ab']==1)
										总额(含AB盘):
									...
									<td><input type="radio" class='ifok' name='ifok{{$vo['ftype']}}' value="1" @if($vo['ifok']==1)checked @endif />启用
										<input type="radio" class='ifok' name='ifok{{$vo['ftype']}}' value="0" @if($vo['ifok']==0)checked @endif />停用
									</td>
								</TR>
								@endif
								@endforeach
				...
				<div id="quickSetting">快速设置： <span class="input_panel">注额：<input type="number" class="integer"
							min="0"></span>&nbsp;&nbsp;<label><input type="radio" name="set"
							value="1">启用</label><label><input type="radio" name="set" value="0"
							checked="checked">停用</label>&nbsp;&nbsp;<input type="button" value="确定"><input type="button"
						gid="{{$gid}}" class="yiwotongbu" value="以我同步{{$flname}}" style="margin-left:5px;display: none;">
				</div>
				<div id="quickSettingb"><BR />
					补货盘口:
				...
				<div class="control">
					<input type="button" value="保存" class="button send"><input type="button" value="取消"
						class="button cancel">
				</div>
