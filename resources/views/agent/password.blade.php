...
<head>
    ...
    <script type="text/javascript">var STATUS= {{$status}};</script>
</head>
<body>
<div class="main">
    <div class="top_info">
    ...
    <div class="contents">
        <table class="list data_table info_table">
            @if($status == 1 || $status == 2)<input type="hidden" id="url" value="login" />@endif
            <tr><th>原始密码</th><td><input id="oldPassword" type="password" class="input" /></td></tr>
            <tr><th>新设密码</th><td><input id="password" type="password" class="input" />＊6-20字符必须由大小写字母和数字组合组成</td></tr>
            <tr><th>确认密码</th><td><input id="ckPassword" type="password" class="input" /></td></tr>
        </table>
    </div>
    <div class="data_footer control">&nbsp;<input class="button" type="button" value="确定修改" onclick="changePassword()" />&nbsp;</div>
</div>
</body>
</html>
