    ...
    <div class="contents">
        <table class="data_table list user_list">
            ...
            <tr>
                ...
                <td class="op"><a href="sub?passname={{$sub['username']}}" class="modify">修改</a> <a href="../loginLogs?id={{$sub['username']}}"
                                                                                        class="login_log info">日志</a>
                </td>
                <td class="status">
                    <input type="button" class="ss{{$sub['status']}}" onclick="showChangeStatusPanel(this,'{{$sub['username']}}',{{$sub['status']}})" value="{{$sub['statusstr']}}"/></td>
            </tr>
            @endforeach
            </tbody>
        </table>
