...
<head>
    ...
    <script type="text/javascript" src="/default/js/water.js?v=1"></script>
</head>
<body class="tax_param">
<div class="main">
    <div class="top_info">
			<span class="title">{{$layername}} <span>{{$username}}（{{$name}}）</span> -&gt; 新增{{$water_name}}模板
		</span>
    ...
    <div class="data_panel">
        ...
        <table class="data_table at_0">
            ...
            <tbody>
            @foreach($lot['waterlist'] as $row)
            <tr>
                @foreach($row as $val)
                <th>{{$val['ckname']}}</th>
                <td class="color t_{{$val['k']}}"><input class="tax" name="{{$lot['lottery']}}_{{$val['ck']}}" value="{{$val['water']}}"/></td>
                @endforeach
            </tr>
            @endforeach
            </tbody>
...
<div class="data_footer control"><input id="id" type="hidden" value="{{$tid}}"/>
    <span>模板名称：<input id="name" class="name" value="{{$tname}}"/></span>
    <input type="button" value="保存" onclick="saveTemplate()" class="button"/>
