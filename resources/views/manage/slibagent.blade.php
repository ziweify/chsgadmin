...
<html xmlns="http://www.w3.org/1999/xhtml" @if($rkey==0)oncontextmenu="return false" @endif>
	<head>
		...
		<script language="javascript" src="/xypone/default/js/default/jsagent/slibnewagent.js?v=1111"></script>
		<style type="text/css">
			.flys {
				background: yellow
			}
		...
		<div class="main">
			<ul class="menu_sub">
				...
				<li id="XYFT" class="menu_sub_links">
                    @foreach($b as $key=>$vo)
					<a bid="{{$vo['bid']}}" href="javascript:void(0)" target="frame" class="@if($key==0)selected @endif n{{$vo['bid']}}">{{$vo['name']}}</a> @if($key!=count($b)-1)|@endif
                    @endforeach
				</li>
			</ul>
			<div class="top_info">
			...
			<div class="top_info psets">
				...
				<select class="plmode">
					...
					@if($pself!=1)<option value="2">调节值</option>@else<option value="4">默认赔率</option>@endif
				</select>
