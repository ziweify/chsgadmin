...
<head>
    ...
    <script language="javascript" src="/xypone/default/js/default/jshide/seditsonmyadmin.js?v=11221111"></script>
    <style>
        td,
        th {
            height: 30px;
...
<div class="main">
    ...
    <div class="contents">
        <table class="data_table data_list list user_list">
            <thead>
            <TR>
                ...
                <th>最后登录时间</th>
                <th>最后修改密码时间</th>
                <th><input class='add btn1 btnf' value='新增子帐号' id="add{$data[0]['userid']}" type='button'/></th>
            </TR>
            </thead>
            @foreach($data as $vo)
            <tr>
                <td class="online">
                    @if($vo['online']==0)
                        <span class='s0'></span>
                    @else
                        <span class='s1 zhuxiao' title='注销'></span>
                    @endif
                </td>
                <td>{{$vo['username']}}</td>
                <td>{{$vo['regtime']}}</td>
                ...
                <td style="width:180px;">
                    ...
                    <input type='button' class='del btn1 btnf' value='删除' id="del{{$vo['userid']}}"
                           uid="{{$vo['userid']}}"/>
                    @endif
                    <input class='edit btn1 btnf' id="edit{{$vo['userid']}}" value='修改密码' uid="{{$vo['userid']}}"
                           username='{{$vo['username']}}' type='button'/>
                    <input class='record btn1 btnf' value='记录' uid='{{$vo['userid']}}' username='{{$vo['username']}}'
                           id="showrecord{{$vo['userid']}}" type='button'/>
...
<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-front ui-draggable ui-resizable addtb" tabindex="-1"
     role="dialog" aria-describedby="shares" aria-labelledby="ui-id-1"
     style="position: absolute; height: 200.28px; width: 360px; top: 257px; left: 564.5px; display:none; right: auto; bottom: auto;">
    <div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix ui-draggable-handle"><span
            id="ui-id-1" class="ui-dialog-title">2988316036# 占成明细</span>
        <button type="button"
    ...
    <div id="shares" class="popdiv ui-dialog-content ui-widget-content"
        ...
        <table class="data_table">
            ...
            <TR>
                <Th>子帐号</Th>
                <TD><label id='user0'>{{$username}}</label><input type="text" id='user1' maxlength="4" class="txt1"
                                                                style="width:40px;"/><input type="hidden" id='action'
                                                                                            value="add"/></TD>
            </TR>
            <TR>
                <Th>密码</Th>
            ...
            <TR>
                <th colspan="2" class="tcenter"><input type="button" id='addbtn' class="btn1 btnf" value="新增子帐号"/>&nbsp;<input
                        type="button" id='closebtn' class="btn1 btnf" value="关闭"/></th>
            </TR>
            </tbody>
        </table>
    </div>
