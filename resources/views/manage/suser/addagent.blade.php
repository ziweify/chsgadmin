<table class="data_table info_table user_panel addtb list">
	...
	<tbody>
		...
		<tr>
			<th>{{$layername}}帐号：</th>
			<TD>
				<input type="text" name='username' id='username' class="input" maxlength="{{$namelength}}" /><span
					id="usernameMsg"></span>&nbsp;&nbsp;<select name='status' class='hides' layer='{{$layer}}'
					id='status'>
		...
		<TR class='hides'>
			<th>帐户类型：</th>
			<TD>
				<select name="ifagent" id=ifagent>
		...
		<tr>
			...
			<td>
                ...
                @elseif($fudong==0)
				<label><input disabled type="radio" name="fudong" value="0" class="fudong" checked="checked">信用模式</label>
				<label><input disabled type="radio" name="fudong" class="fudong" value="1">现金模式</label>
				@else
				<label><input disabled type="radio" name="fudong" value="0" class="fudong">信用模式</label>
				<label><input disabled type="radio" name="fudong" class="fudong" value="1" checked="checked">现金模式</label>
				@endif
			</td>
		</tr>
		...
		@endif
			<TR>
				...
				<TD>
                    @foreach($pan as $vo)
					<input class="pantype" type="checkbox" value="{{$vo}}" />{{$vo}}
					@endforeach
            ...
            <tr>
                <th>剩余占城归属</th>
                <td>
                    <label><input type="radio" name="zcobj" value="1" class="zcobj" checked="checked">集团</label>
                    <label><input type="radio" name="zcobj" class="zcobj" value="2">一级代理</label>
                </td>
            </tr>
			@endif
            @if($plc == 1)
...
<table class="data_table info_table share_panel input_panel addtb2 list">
	...
	<tbody>
		...
		<tr @if($vo['ifok']==0) style="display:none" @endif>
			...
			<td>
				<input type="text" class="upzc share" maxzc='{{$vo['zc']}}' value='{{$vo['zcmin']}}' />(0% 至 {{$vo['zc']}}%)
			</td>
			<td><input type="text" class="zcmin share" maxzc='{{$vo['zc']}}' value='0' />%</td>
			<td>
			...
			<td>
				<input type="text" class="flyzc share" maxzc='{{$vo['flyzc']}}' value='{{$vo['flyzc']}}' />(0% 至{{$vo['flyzc']}}%)
			</td>
		</tr>
		@endforeach
	</tbody>
...
<table class="data_table info_table share_panel input_panel addtb2 list">
	...
	<tbody>
		...
		<tr @if(($vo['typeid']==1 && $config['fasttype']==0))style='display:none' @endif @if(($vo['typeid']==0 && $config['slowtype']==0)) style='display:none' @endif>
			...
			<td>
				<input type="text" class="zc share" maxzc='{{$vo['zc']}}' value='{{$vo['zc']}}' />(最大{{$vo['zc']}}%)
			</td>
			<td>
				<select class="flytype" val='{{$vo['flytype']}}'>
                    @if($vo['flytype']==0)
