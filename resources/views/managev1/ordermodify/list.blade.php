									...
									<td valign="middle">
										<table cellspacing="0" cellpadding="0" width="100%" border="0">
											...
											<tr>
												...
												<td class="F_bold" width="49%" align="left">&nbsp;注单查询</td>
						...
						<td align="center">
								...
								<tr>
									...
									<td style="background-color: white;">
										<div style="overflow: hidden; float: left; padding: 0px; margin: 0px;"
											...
											<input disabled value="{{$sdate[10]}}" id="txtenddate"
								...
								<tr class="tdstandboll">
									...
									<td align="left" class="t_Edit_td">
										<div style="display: flex;align-items: center;">
											...
											<input @click="editzdall" style="margin-left: 10px" type="button" value="修改选中" onmouseover="this.className='btn2m'" onmouseout="this.className='btn2'" class="btn2"/>
											<input @click="delxzzd" style="margin-left: 10px" type="button" value="删除选中" onmouseover="this.className='btn2m'" onmouseout="this.className='btn2'" class="btn2"/>
										</div>
									</td>
								</tr>
							</table>
							<table class="t_list" border="0" cellpadding="0" cellspacing="1" style="margin-top: 5px; line-height: 25px; width: 100%;">
								...
								<tr v-if="list.length > 0" v-for="(item,index) in list" :class="{t_list_tr_0:index%2==0,t_list_tr_1:index%2!=0}" class="nr" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
									...
									<td><a @click="editzd(item)" href="javascript:void(0);" class="czuan">修改</a>&nbsp;<a @click="delzd(item)" href="javascript:void(0);"  class="czuan">删除</a></td>
									<td>@{{item.game}}</td>
									<td>@{{item.qishu}}</td>
									<td>@{{item.duser}}</td>
									<td>@{{item.code}}</td>
									<td>@{{item.xtype}}</td>
									<td>
										<template v-if="(item['fl'] == 107 && item['bids'] != 23378805) || (item['fl'] == 101 && item['bids'] == 23378755)">
											<template v-if="item['fl'] == 107">
												<select v-model="item.sid">
													<option v-for="bitem in flbig107" :value="bitem">@{{bitem}}</option>
												</select>:
												<select v-model="item.pid">
													<option v-for="sitem in flsis107" :value="sitem">@{{sitem}}</option>
												</select>
											</template>
											<template v-if="item['fl'] == 101">
												<select v-model="item.sid">
													<option v-for="bitem in flbig101" :value="bitem">@{{bitem}}</option>
												</select>:
												<select v-model="item.pid">
													<option v-for="sitem in flsis101" :value="sitem">@{{sitem}}</option>
												</select>
											</template>
										</template>
										<template v-else>
                                            @{{item.bid}}-@{{item.sid}}-@{{item.cid}}:<input class="s60input" v-model="item.pid" />
										</template>
									</td>
									<td>@{{item.abcd}}</td>
									<td>
										<select v-model="item.z">
											<option v-for="zitem in stypearr" :value="zitem.value">@{{zitem.title}}</option>
										</select>
									</td>
									<td><input class="s40input" v-model="item.je" disabled /></td>
									<td><input class="s60input" v-model="item.peilv1" disabled /></td>
									<td><input class="s40input" v-model="item.points" disabled /></td>
									<td><input v-model="item.time" disabled /></td>
									{{--<td>
...
<script type="text/javascript">
	var vm = new Vue({
		...
		methods: {
			...
			delxzzd:function (){
				//拼接字段为checkbox为true的字段，用,分割
				var ids = [];
				for(var i=0;i<this.list.length;i++){
					if(this.list[i].checkbox){
						ids.push({code:this.list[i].code,dates:this.list[i].dates});
					}
				}
				if(ids.length <= 0){
					hwProwin.errorInfo({message: "请选择要删除的数据"});
					return;
				}
				this.delzdaction(ids);
			},
			delzd:function (item){
				this.delzdaction([{code:item.code,dates:item.dates}]);
			},
			delzdaction:function (ids){
				var self = this;
				$.ajax({
					type: 'POST',
					url: 'delzddg',
					data: {str:JSON.stringify(ids)},
					dataType: 'json',
					cache: false,
					success: function(m) {
					    if(m.code == 0){
