...
<head>
    ...
    <script type="text/javascript">
        ...
        function bf(){
            window.open("/agent/bfdangqi?lottery="+lottery)
        }
    </script>
    <script type="text/javascript" src="/default/js/lotterys.js"></script>
</head>
<body class="L_{{$template}}" onload="startTime()">
<div class="main">
    <div class="top_info">
        ...
        <div class="op">
            ...
            <select id="username">
                @foreach($users as $user)
                    ...
                    @else
                    <option value="{{$user->username}}">[{{$user->layername}}]{{$user->username}}({{$user->name}})</option>
                    @endif
                @endforeach
            </select>
            @if($is_open_dqbf == 1)
            <input type="button" onclick="bf()" style="cursor: pointer" value="备份当前数据" />
            @endif
        </div>
        <div class="right">
            <div id="drawInfo">
