...
$(function () {
    ...
    if (MULTIPLE) {
        ...
        function h(u) {
            ...
            var a = getBet(u);
            if (a) {
                parent.showBets({lottery: lottery, drawNumber: t.drawNumber, bets: [a]})
            }
        }
        ...
        if (parent && parent.QuickAmountEnalbed) {
            ...
            parent.QuickAmount = "";
            sessionStorage.setItem("QuickAmount", "")
        }
    ...
    var s = {
        titleConverter: function (a) {
            ...
            return a
        }, interval: 10, changlong: $("#changlong tbody"), onChangLongClick: function (t) {
            ...
            url: "odds", data: {lottery: lottery, games: games}, success: function (a) {
                showOdds(a)
            }
        }
    };
    ...
    if (f && m) {
        ...
        $("label.checkdefault input").prop("checked", f == "true" ? true : false);
        if (parent && f == "true") {
...
function getBet(g, m) {
    ...
    f = (f.length > 1) ? f[0] : "";
    var d = g.split("_");
    var b = {title: f, text: h, game: d[0], contents: d[1], odds: q};
...
function getMultipleBets() {
    var n = Math.floor(Number($(".quickAmount input").val()));
    if (n <= 0 || isNaN(n)) {
        return false
    }
    var f = MULTIPLE[0];
...
function bet() {
    ...
    parent.showBets({lottery: lottery, drawNumber: h.drawNumber, bets: a, fastBets: false})
}
...
function getGgBet() {
    ...
    e += "@" + oddsDetail;
    var b = {title: h, text: g, game: gamegg, contents: e, odds: a, amount: d};
    return b
}
...
function repeatbet() {
    ...
    parent.showRepeatBets({lottery: lottery, drawNumber: a.drawNumber})
};
