...
$(function () {
    ...
    $(".detail a").click(function () {
        var b = $("#shares");
        if (b.length == 0) {
            b = $('<div id="shares">').addClass("popdiv");
            b.dialog({autoOpen: false, width: 300, maxHeight: 400})
        }
        var c = $(this).attr("bid");
        var a = $(this).attr("drawDate");
        b.dialog("option", {title: c + "# 占成明细"});
        b.empty();
        b.append('<span class="loading">载入中……</span>').load(url, {code: c, drawDate: a});
        b.dialog("open")
    });
