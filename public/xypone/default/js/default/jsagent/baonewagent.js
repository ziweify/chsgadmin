	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<td class='result color'>" + getResult2(0-m[i]['yk'],1) + "</td>";
				str += "<td>" + m[i]['mezcp'] + "</td>";
				...
				if (m[i]['fly'] == '1' || m[i]['fly'] == '2') {
                    ...
                    //console.log(plc,getResult(Number(m[i]['mezhong']) + Number(m[i]['sendzhong']) - Number(m[i]['zhong']), 2));
...
function agentnew() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (qishu == '') {
                $(".main2 .title").html(layername[layer - 1] + "（" + cusername + "） - 交收报表 [" + start + " — " + end + "] ")
			} else {
				$(".main2 .title").html(layername[layer - 1] + "（" + cusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
			}
			$(".main2 .syname").html(layername[layer - 1] + "输赢");
			if (username != '') {
			...
			for (i = 0; i < ml; i++) {
                ...
                if(m[i]['fly'] == 0){
                    str += "<td>" + m[i]['layername'] + "</td>";
                    str += "<td><a  href='javascript:void(0);' class='u'  ifagent='" + m[i]['ifagent'] + "' uid='" + m[i]['userid'] + "' layer='" + m[i]['layer'] + "'  ttype='" + m[i]['ttype'] + "'  >" + m[i]['user'] + "</a></td>";
                    str += "<td>" + m[i]['name'] + "</td>";
                    str += "<td>" + m[i]['money'] + "</td>";
                }else if(m[i]['fly'] == 1){
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (j = 0; j < ml; j++) {
				...
				obj.find("caption").html(m['game'][j]['gname']);
				if (qishu == '') {
                    obj.find(".title").html(layername[layer - 1] + "（" + cusername + "） - 交收报表 [" + start + " — " + end + "] ")
				} else {
					obj.find(".title").html(layername[layer - 1] + "（" + cusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
				}
				obj.find(".syname").html(layername[layer - 1] + "输赢");
				var str = '';
				$(".plc").hide();
                $(".tax").hide();
