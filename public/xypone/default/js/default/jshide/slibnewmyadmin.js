...
function myready() {
    ...
    //console.log('ggggg',Number($("#reloadtime").val()));
    ...
    $("#yiwotongbu").click(function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/yiwotongbu',
            cache: false,
            data: "xtype=gid=" + ngid + "&epl=" + epl,
            success: function (m) {
                if (Number(m) == 1) {
                    alert('ok');
    ...
    $(".onepeilvtb .onesend").click(function () {
        ...
        $.ajax({
            ...
            cache: false,
            async: true,
            data: {pl: pl, epl: epl, ab: ab, abcd: abcd},
            success: function (m) {
                if (Number(m) == 1) {
                    var val = peilv1;
                    if (epl == 4) {
    ...
    $("#psetpost").click(function () {
        if ($(".duopl").length == 1) {
            ...
            $.ajax({
                ...
                url: '/admin/setpeilvall',
                cache: false,
                data: {pl: plstr, epl: epl, ab: ab, abcd: abcd},
                success: function (m) {
                    if (Number(m) == 1) {
                        var val;
                        $("td.byellow").each(function () {
...
function qiu(n, bname, sname) {
    if (isNaN(n) || bname == '2字组合' || bname == '2字和数' || bname == '3字组合' || bname == '3字和数') {
        ...
        if (bname == '番摊') {
            ...
            if (n > 100) {
                n = "<img src='/xypone/default/imgn/sz" + (n % 10) + "1.png' /><img src='/xypone/default/imgn/sz" + (n % 10) + "1.png' /><img src='/xypone/default/imgn/sz" + (n % 10) + "1.png' />"
            } else if (n > 10) {
                n = "<img src='/xypone/default/imgn/sz" + n.substr(0, 1) + "1.png' /><img src='/xypone/default/imgn/sz" + (n % 10) + "1.png' />"
            } else {
...
function zcxxclick(tb) {
    ...
    $("." + tb + " a.xxc").click(function () {
        ...
        sstr += "&sorttype=" + sorttype + "&xtype=" + xtype + "&pid=" + pid + "&con=" + con + "&flys=" + flys;
        ...
        $.ajax({
            ...
            success: function (m) {
                ...
                $(".xxtb a.mx").click(function () {
                    ...
                    $("div.mx").show();
                    var posi = $(this).position();
					$('div.mx').css({top:'30%',left:'50%',margin:'-'+($('div.mx').height() / 2)+'px 0 0 -'+($('div.mx').width() / 2)+'px'});
