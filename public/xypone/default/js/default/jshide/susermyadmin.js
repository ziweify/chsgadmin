...
function myready() {
	...
	//$(".cpasstb .cpasssend").click(function() {
		...
		$.ajax({
			type: 'POST',
			url: '/admin/susercpass',
			cache: false,
			data: 'pass1=' + pass1 + "&uid=" + uid,
			success: function(m) {
				$("#test").html(m);
				m = Number(m);
				if (m == 1) {
	...
	$("#tongbugamecs").click(function() {
		$.ajax({
			type: 'POST',
			url: '/admin/susertongbugamecs',
			cache: false,
			data: {},
			success: function(m) {
				if (Number(m) == 1) alert('同步成功');
			}
		});
		return false
	});
	...
	$(".edutb .setmoney").click(function() {
		...
		$.ajax({
			type: 'POST',
			url: '/admin/susersetmoney',
			async: false,
			cache: false,
			data: 'uid=' + uid + '&etype=' + etype + '&types=' + types + '&je=' + je+"&pass="+pass,
			success: function(m) {
	...
	$.ajax({
		...
		success: function(m) {
			...
			$(".user_tb a.showson").click(function() {
				var uid = $(this).parent().parent().attr('uid');
				window.location.href = '/admin/editsonpage?uid=' + uid;
				return false
			});
			$(".user_tb a.moneylog").click(function() {
				var uid = $(this).parent().parent().attr('uid');
				window.location.href = '/admin/susermoneylog?uid=' + uid;
				return false
			});
	...
	$.ajax({
		...
		success: function(m) {
			...
			$(".utb .edit").click(function() {
				...
				var ifexe = $("#ifexe").val();
				if(ifexe == undefined || ifexe == null)ifexe = 0;
				var pself = $("#pself").val();
				if(pself == undefined || pself == null)pself = 0;
				var cssz = $("#cssz").val();
				if(cssz == undefined || cssz == null)cssz = 0;
				var ifagent = $("#ifagent").val();
				var fid = $("#fid").val();
				var status = $("input[name='status']:checked").val();
				var zcobj = $("input[name='zcobj']:checked").val();
				if(zcobj == undefined || zcobj == null)zcobj = 0;
				var plc = $("#plc").val() ? $("#plc").val() : 0;
				var str = "ifagent=" + ifagent + "&ifexe=" + ifexe + "&plc=" + plc;
				str += "&defaultpan=" + defaultpan + "&pan=" + pan;
				str += "&userpass=" + userpass + "&name=" + name + "&userid=" + userid;
				str += "&fid=" + fid + "&status=" + status + "&pself=" + pself;
				str += "&cssz=" + cssz+"&zcobj="+zcobj;
				var gamecs = "{";
				j = 0;
...
function addfunc(types) {
	...
	$(".utb input.add").click(function() {
		...
		var maxren = $("#maxren").val() ? $("#maxren").val() : 0;
		var pan = '[';
		var j = 0;
		$(".pantype:checked").each(function() {
			if (j > 0) pan += ',';
		...
		pan += "]";
		var defaultpan = $("#defaultpan").val();
		var ifexe = $("#ifexe").val() ? $("#ifexe").val() : 0;
		var pself = $("#pself").val() ? $("#pself").val() : 0;
		var cssz = $("#cssz").val() ? $("#cssz").val() : 0;
		var ifagent = $("#ifagent").val();
		var fid = $("#fid").val();
		//var layer = $("#layer").val();
		var plc = $("#plc").val() ? $("#plc").val() : 0;
		...
		var str = "ifagent=" + ifagent + "&ifexe=" + ifexe;
		...
		str += "&fid=" + fid + "&pself=" + pself + "&liushui=" + liushui;
		str += "&cssz=" + cssz;
		var gamecs = "{";
		j = 0;
		$(".addtb2 tbody tr").each(function(i) {
...
function deluser(ustr) {
	...
	$.ajax({
		type: 'POST',
		url: '/admin/suserdeluser',
		cache: false,
		data: 'ustr=' + ustr + "&pass=" + pass,
		success: function(m) {
			if (Number(m) == 1) {
				$(".user_tb").find("input:checkbox").each(function(i) {
