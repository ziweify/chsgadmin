...
function savewater_start_bishu() {
    $.ajax({
        url: "savewater_start_bishu",
        type: "POST",
        data: {water_start_bishu:$("#water_start_bishu").val()},
        success: function (e) {
...
function resetUserTax(e,d) {
    confirm("是否要恢复【" + d + "】的今日"+water_name+"？请再三确认！\n（包括报表与额度）") && confirm("是否要恢复【" + d + "】的今日"+water_name+"？请再次确认！\n（包括报表与额度）") && confirm("是否要恢复【" + d + "】的今日"+water_name+"？请最后确认一次！\n（包括报表与额度）") && postCall("resetUserWater", {userid: e}, function () {
        location.reload()
    })
}
...
function saveParam() {
    ...
    $(".data_panel input.changed").each(function () {
        ...
        a.push({lottery: t[0], game: t[1], water: e.val()})
    }), 0 == a.length ? alert("没有任何变更的"+water_name) : confirm("是否保存 " + $("#username").val() + " 的"+water_name+"设置？") && (0 == $(".data_footer .cur_tpl").length || confirm("警告：当前用户已与模板关联，修改用户"+water_name+"会取消与模板关联，模板设置变更后不会自动更新当前用户的"+water_name+"！\n是否确认继续保存？")) && $.ajax({
        url: "saveParam",
        type: "POST",
        loading: !0,
        data: {userid: $("#userid").val(), params: JSON.stringify(a)},
        success: function (e) {
            e.success ? (alert(water_name+"设置已成功保存。"), back()) : alert("设置错误：" + e.message)
        },
        error: function () {
            alert("网络错误，请稍后重试")
        }
...
function saveTemplate() {
    var a, e = $("#name").val();
    e ? confirm("是否保存"+water_name+"模板【" + e + "】？") && (a = [], $(".data_panel input.changed").each(function () {
        var e = $(this), t = e.attr("name").split("_");
        a.push({lottery: t[0], game: t[1], water: e.val()})
    }), $.ajax({
        url: "saveTemplate",
        type: "POST",
        loading: !0,
        data: {tid: $("#id").val(), name: e, params: JSON.stringify(a)},
        success: function (e) {
            e.success ? (alert(water_name+"模板已成功保存。"), back()) : alert("保存错误：" + e.message)
        },
        error: function () {
            alert("网络错误，请稍后重试")
        }
...
function delTemplate(e, t, a) {
    t = "是否删除"+water_name+"模板【" + t + "】？";
    a && (t += "\n警告：该模板已有 " + a + " 个用户引用，用户"+water_name+"将转为“自定义”，保留当前模板设置！"), confirm(t) && postCall("delTemplate", {tid: e}, function () {
        location.reload()
    }, "删除成功", "删除错误")
}
...
$(function () {
    ...
    o.length && (e = 20, o.text("刷新 (" + e + ")"), window.setInterval(function () {
        ...
        confirm("是否确定停止全部下线会员的"+water_name+"？（不恢复额度、报表及已结注单，仅停用"+water_name+"）") && postCall("resetWater", null, function () {
            location.reload()
        })
        ...
        var e = $(this).parent().parent(), t = e.find(".water_status"),s = e.find(".water_auto_close");
        $.ajax({
            url: "saveWater",
            type: "POST",
            loading: !0,
            data: {userid: t.attr("name"), enabled: t.prop("checked") ? 1 : 0, water_auto_close: s.prop("checked") ? 1 : 0, tid: e.find("select").val()},
            success: function (e) {
        ...
        l.filter(":checked").each(function () {
            e.push($(this).val())
        }), 0 != e.length && confirm("是否恢复选中注单的"+water_name+"？（恢复报表、返回额度）") && $.ajax({
            url: "resetBetWater",
            type: "POST",
            loading: !0,
            ...
            success: function (e) {
                e.success ? (alert("注单"+water_name+"已成功恢复。"), location.reload()) : alert("恢复错误：" + e.message)
            },
            error: function () {
                alert("网络错误，请稍后重试")
            }
