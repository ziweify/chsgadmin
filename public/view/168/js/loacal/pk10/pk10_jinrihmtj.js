function choseNum() {
    yzHaomaduan();
    var t = $("#colorSet").find(".param");
    $(t).each(function () {
        if (!$(this).is(":hidden")) {
            var t = $(this).find(".input1").val(), s = $(this).find(".input2").val(), e = $("#table_cltj td");
            "sid1" == $(this).attr("id") ? $(e).each(function () {
                var e = $(this).text(), a = 1 * e <= 1 * s;
                1 * e >= 1 * t && a ? (config.ifdebug, "rgb(153, 153, 153)" == $(this).css("color") && $(this).css({color: "#f1010a"})) : "rgb(241, 1, 10)" == $(this).css("color") && $(this).css({color: "#999999"})
            }) : "sid2" == $(this).attr("id") ? $(e).each(function () {
                var e = $(this).text(), a = 1 * e <= 1 * s;
                1 * e >= 1 * t && a ? (config.ifdebug, void 0 != $(this).css("color") && $(this).css({color: "#0092dd"})) : "rgb(0, 146, 221)" == $(this).css("color") && $(this).css({color: "#999999"})
            }) : "sid3" == $(this).attr("id") && $(e).each(function () {
                var e = $(this).text(), a = 1 * e <= 1 * s;
                1 * e >= 1 * t && a ? (config.ifdebug, void 0 != $(this).css("color") && $(this).css({color: "#07bf00"})) : "rgb(7, 191, 0)" == $(this).css("color") && $(this).css({color: "#999999"})
            })
        }
    })
}
...
function yzHaomaduan() {
    var t = [15, 30], s = [31, 40], e = [41, 50], a = ($(this).attr("id"), $("#star1").val()), i = $("#end1").val(),
        l = $("#star2").val(), n = $("#end2").val(), c = $("#star3").val(), d = $("#end3").val();
    1 * a < t[0] ? $("#star1").val(t[0]) : 1 * a >= t[1] && $("#star1").val(t[0]), 1 * i < t[0] ? $("#end1").val(t[1]) : 1 * i >= t[1] && $("#end1").val(t[1]), 1 * l < s[0] ? $("#star2").val(s[0]) : 1 * l >= s[1] && $("#star2").val(s[0]), 1 * n < s[0] ? $("#end2").val(s[1]) : 1 * n >= s[1] && $("#end2").val(s[1]), 1 * c < e[0] ? $("#star3").val(e[0]) : 1 * c >= e[1] && $("#star3").val(e[0]), 1 * d < e[0] ? $("#end3").val(e[1]) : 1 * d >= e[1] && $("#end3").val(e[1])
}
...
function doCheck(t, s) {
    var e = $("." + t + "  .item_" + s + " .lz_table_head td .txtNum").val(),
        a = $("." + t + "  .item_" + s + " .lz_table_head td .secType").val(), i = Number("0" + e), l = 0;
    ...
    }), $("." + t + "  .item_" + s + " .lz_table_head td .sec_count").html(l), $("." + t + "  .item_" + s + "  .lz_table_head td .count").each(function () {
        var e = $(this).attr("data"), a = 0;
        $("." + t + "  .item_" + s + " .lz_table_con td p").each(function () {
            $(this).html() == e && a++
        }), $(this).html(a)
    });
    var n = 0, c = $("." + t + "  .item_" + s + " .lz_table_con td:first-child p:last ");
    c.css("font-weight", "bold");
    var d = setTimeout(function () {
        c.fadeOut(100).fadeIn(100), 1 == ++n && (d = setInterval(arguments.callee, 600)), 5 == n && window.clearInterval(d)
    }, 1e3)
}
...
function listData() {
    ...
    var t = $("#tabdsdxcl").find("span[class*='xxselected']").find("i").attr("title");
    $("#tabdsdxcl").find("span[class*='xxselected']").find("i").text();
    0 == t ? $("#dxdsmc").find("li[class*='checked']").find("span").text() : $("#lhmc").find("li[class*='checked']").find("span").text(), $.ajax({
...
function loadLongData(t) {
    ...
    for (var s = 0, e = data.length; s < e; s++) {
        var a = minci(data[s].rank, "rank"), i = minci(data[s].state, "state"),
            l = data[s].count >= 5 ? "<span style='color:#f11821'>" + data[s].count + "</span>" : "<span>" + data[s].count + "</span>",
            n = "<li>第<span>" + a + "</span>名：&nbsp;&nbsp;<span>" + i + "</span>&nbsp;&nbsp;" + l + "期</li>";
        11 != data[s].rank && 1 != data[s].rank && 2 != data[s].rank || (n = "<li><span>" + a + "</span>：&nbsp;&nbsp;<span>" + i + "</span>&nbsp;&nbsp;" + l + "期</li>"), $("#cltxul").append(n)
    }
}
...
function lmmsMcMethod(t, s) {
    var e = $(s);
    if (e.removeClass("hoverli"), "all" == t) (lmmssxmc = []).length >= 10 || $($("#lmms").find(".sinli")).each(function (t) {
...
function excuteZhmsSelect(t, s) {
    for (var e = [], a = 0, i = t.length; a < i; a++) for (var l = 0, n = s.length; l < n; l++) if (t[a] >= 6) {
        if (s[l] > 2) continue;
        e.push(t[a] + "" + s[l])
    } else e.push(t[a] + "" + s[l]);
    config.ifdebug, $(".box .item").hide(), $(e).each(function (t) {
        $("#box" + e[t]).show()
    })
}
...
function createHtmlList(t) {
    var s = null;
    "object" != (void 0 === t ? "undefined" : _typeof(t)) ? s = JSON.parse(t) : (s = JSON.stringify(t), s = JSON.parse(s)), forRank(s.result.data)
}

function forRank(t) {
    $("#table_cltj tbody").empty(), $(t).each(function (t) {
        var s = "", e = this.code < 10 ? "numss0" + this.code : "numss" + this.code;
        s += "<td><span style='display:inline-block;width:26px;height:26px' class='" + e + "'></span></td>";
        var a = this.list;
        $(a).each(function (t) {
            s += "<td>" + this.accumulate + "</td><td>" + this.missing + "</td>"
        }), $("#table_cltj tbody").append("<tr>" + s + "</tr>")
    })
}

function boxList(t, s) {
    var e = "";
    return 1 == t && 1 == s.state ? e = "" + t + s.state : 1 == t && 2 == s.state ? e = "" + t + s.state : 1 == t && 3 == s.state ? e = "" + t + s.state : 2 == t && 1 == s.state ? e = "" + t + s.state : 2 == t && 2 == s.state ? e = "" + t + s.state : 2 == t && 3 == s.state ? e = "" + t + s.state : 3 == t && 1 == s.state ? e = "" + t + s.state : 3 == t && 2 == s.state ? e = "" + t + s.state : 3 == t && 3 == s.state ? e = "" + t + s.state : 4 == t && 1 == s.state ? e = "" + t + s.state : 4 == t && 2 == s.state ? e = "" + t + s.state : 4 == t && 3 == s.state ? e = "" + t + s.state : 5 == t && 1 == s.state ? e = "" + t + s.state : 5 == t && 2 == s.state ? e = "" + t + s.state : 5 == t && 3 == s.state ? e = "" + t + s.state : 6 == t && 1 == s.state ? e = "" + t + s.state : 6 == t && 2 == s.state ? e = "" + t + s.state : 7 == t && 1 == s.state ? e = "" + t + s.state : 7 == t && 2 == s.state ? e = "" + t + s.state : 8 == t && 1 == s.state ? e = "" + t + s.state : 8 == t && 2 == s.state ? e = "" + t + s.state : 9 == t && 1 == s.state ? e = "" + t + s.state : 9 == t && 2 == s.state ? e = "" + t + s.state : 10 == t && 1 == s.state ? e = "" + t + s.state : 10 == t && 2 == s.state ? e = "" + t + s.state : 11 == t && 1 == s.state ? e = "" + t + s.state : 11 == t && 2 == s.state && (e = "" + t + s.state), e
}

function forRankYibu(t, s) {
    var e = $("#box" + boxList(t, s)), a = "", i = "", l = e.find(".tablebox td:nth-child(1) p:last-child").text();
    e.find(".tablebox td:nth-child(1) p:last-child").css({"font-weight": "normal"});
    var n = s.state, c = "";
    "1" == n ? c = 1 == s.value ? "单" : "双" : "2" == n ? c = 1 == s.value ? "大" : "小" : "3" == n && (c = 1 == s.value ? "龙" : "虎"), "双" != c && "龙" != c && "大" != c || (i = "style='color:#f1020b'");
    ...
    var x = setTimeout(function () {
        u.fadeOut(100).fadeIn(100), 1 == ++f && (x = setInterval(arguments.callee, 600)), 5 == f && window.clearInterval(x)
    }, 1e3)
}
...
$(function () {
    $("#gotop").click(function () {
        ...
        "0" == t ? ($("#dxdsmc").show(200), $("#lhmc").hide(200)) : "1" == t && ($("#dxdsmc").hide(200), $("#lhmc").show(200)), listData()
    ...
    }), listData(), $("#btn1").live("click", function () {
