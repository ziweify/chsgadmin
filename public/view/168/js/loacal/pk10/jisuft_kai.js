...
function iframe() {
    $(".animate iframe").contents().find("#logoimg").attr("src", "images/logo_" + lotCode + ".png"), isfirthload && ($(".animate").find(".loading").fadeOut("slow"), $(".animate iframe").contents().find(".container").hide(), $(".animate iframe").contents().find("#preloader").hide());
...
function creatDataHead(t, e) {
    for (var i = tools.parseObj(t), n = (i = i.result.data).preDrawCode.split(","), o = [], a = 0; a < n.length; a++) {
        if (1 != n[a].charAt(0)) s = n[a].charAt(1); else var s = n[a];
        o.push(s)
    }
    var r = o.toString(), c = tools.operatorTime("" == i.drawTime ? "0" : i.drawTime, i.serverTime);
    if (c <= 0) throw new Error("error");
    !$("#videobox").length < 1 && -1 != $("#videobox").css("z-index") ? ($("iframe")[0].contentWindow.finishgame(r, c, i), setTimeout(function () {
...
$(function () {
    ...
    $("#headdivbox").load("../public/head.html", function () {
        ...
        e.hasClass("checked") && e.removeClass("checked"), i.hasClass("checked") && i.removeClass("checked"), tabarr = [], $("#chakanchfb,#daxiaodsfb").find("li").removeClass("selected"), "today" == t ? (listData(getDateStr(0), ""), $(".jinri").text("今天"), $("#dateframe").find("input").val(getDateStr(0))) : "yesterday" == t ? (listData(getDateStr(-1), ""), $(".jinri").text(currentDay(getDateStr(-1))), $("#dateframe").find("input").val(getDateStr(-1))) : "qianday" == t ? (listData(getDateStr(-2), ""), $(".jinri").text(currentDay(getDateStr(-2))), $("#dateframe").find("input").val(getDateStr(-2))) : "thirty" == t ? listData("", "30") : "sixty" == t ? listData("", "60") : "ninety" == t && listData("", "90")
        ...
        $("iframe")[0].contentWindow.ifopen || ($("#videobox").animate({"z-index": "19999"}, 200, function () {
            $(".content").animate({
                ...
                $(".big").hide(), $(".small").show(), $(".animate").height("575"), iframe(), isfirthload = !1, $("iframe")[0].contentWindow.fun.stateSound(), tools.insertVideo(), tools.setPK10TB()
            })
        ...
        }), $("iframe")[0].contentWindow.ifopen = !1, $("iframe")[0].contentWindow.running.pause(), $("iframe")[0].contentWindow.kaisound.pause(), $("iframe").contents().find("#sound").attr("src", "")
...
var urlbublic = config.publicUrl, lotCode = getParameterByName("code"), boxid = "#pk10", isfirthload = !0;