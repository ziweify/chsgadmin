...
function listData() {
    ...
    0 == t ? $("#dxdsmc").find("li[class*='checked']").find("span").text() : $("#lhmc").find("li[class*='checked']").find("span").text(), $.ajax({
        url: urlbublic + "CQShiCai/queryHistoryDataForNumber.do",
        type: "GET",
...
function forRank(t) {
    $("#dsdxlstable tbody").empty(), $(t).each(function (t) {
        var s = "";
        s += "<td>" + this.date + "</td>";
        var e = this.list;
        $(e).each(function (t) {
            s += "<td>" + this.counts + "</td>"
        }), $("#dsdxlstable tbody").append("<tr>" + s + "</tr>")
    })
}
...
function forRankYibu(t, s) {
    ...
    var d = e.find(".left_count"), h = e.find(".right_count"), r = 1 * d.text() + 1, o = 1 * h.text() + 1;
    if ("单" == c ? d.text(r) : "双" == c ? h.text(o) : "大" == c ? d.text(r) : "小" == c ? h.text(o) : "龙" == c ? d.text(r) : "虎" == c && h.text(o), l == c) a = "<p  " + i + ">" + l + "</p>", e.find(".tablebox td:nth-child(1)").append(a); else {
        var m = "";
...
$(function () {
    $("#gotop").click(function () {
        ...
        var t = $(this).attr("id");
        "today" == t ? listData(getDateStr(0)) : "yesterday" == t ? listData(getDateStr(-1)) : "qianday" == t && listData(getDateStr(-2))
